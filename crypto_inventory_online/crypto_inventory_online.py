...
def get_data_from_opensearch(index_name, query):
    try:
        ...
        response = client.search(index=index_name, body=query, size=response_count['count'])
        return response
...
def map_ciphersuite(ciphersuite_file, response):
    ...
    unique_data = {frozenset((item['origin_ip'], item['response_ip'], item['origin_port'], item['response_port'])): item for item in data}
    data = list(unique_data.values())

    # Mapping
    for c in data:
        c['ciphersuite'] = formatted_cs.get(c['cipher_suite'], {}) if c['cipher_suite'] else "null"
...
def main():
    ...
    zeek_ssl_cipher = map_ciphersuite(ciphersuite_file=cipher_file, response=response)
